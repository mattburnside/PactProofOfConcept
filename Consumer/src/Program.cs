...
//            Console.WriteLine("...Date validation complete. Goodbye.");
            var result = ConsumerApiClient.LogInUser("testuser", "testpassword", baseUri).GetAwaiter().GetResult();
            Console.WriteLine($"Status code is {result.StatusCode}");
            var resultContentText = result.Content.ReadAsStringAsync().GetAwaiter().GetResult();
            Console.WriteLine("Response Text is");
            Console.WriteLine(resultContentText);
        }
